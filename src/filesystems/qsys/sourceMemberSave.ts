...
async function saveRowsToHost(connection: IBMi, uri: vscode.Uri, rows: [number, number, string][], recordLength: number) {
    ...
    try {
        ...
        for (let i = 0; i < rows.length; i += rowsPerBatch) {
            ...
            const bindings = batch.flatMap(row => [row[0], row[1], row[2]]);
            await connection.runSQL(
        ...
        await connection.runSQL([
            ...
            `Insert into ${BACKUP_TABLE} (SRCSEQ, SRCDAT, SRCDTA) Select SRCSEQ, SRCDAT, SRCDTA From ${TARGET_ALIAS}`
        ].join(`;\n`));
...
function buildRows(body: string, recordLength: number, sourceDates: string[]) {
    ...
    for (let i = 0; i < sourceData.length; i++) {
        ...
        rows.push([
            sequence,
            Number(sourceDates[i] ? sourceDates[i].padEnd(6, `0`) : `0`),
            sourceData[i],
        ]);
    }
...
export async function saveSourceMbrToHost(options: SaveSourceMbrOptions) {
    ...
    if (typeof previousBody === `string` && previousBody !== body) {
        try {
            ...
            if (archivePath) {
                connection.appendOutput(`[Source save] Archived previous member content to ${archivePath}.\n`);
            }
        } catch (archiveError) {
