    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(`code-for-ibmi.showAdditionalSettings`, async (server?: Server, tab?: string) => {
        ...
        featuresTab
          ...
          .addCheckbox(`mapepireUseServer`, `Connect to Mapepire over HTTPS`, `When enabled, Code for IBM i will connect over HTTPS to a Mapepire server running on the target IBM i instead of running Mapepire in an SSH channel.`, config.mapepireUseServer)
          .addCheckbox(`mapepireAllowSelfCert`, `Allow all certificates`, `When enabled, allows either self-signed certificates or certificates from a CA when connecting to Mapepire over HTTPS.`, config.mapepireAllowSelfCert)
          .addInput(`mapepireServerPort`, `Mapepire Server HTTPS port`, `The TCP port number Code for IBM will use when connecting to Mapepire over HTTPS.`, { default: String(config.mapepireServerPort), inputType: "number", min: 1, max: 65535 })
