      ...
      vscode.commands.registerCommand(`code-for-ibmi.showAdditionalSettings`, async (server?: Server, tab?: string) => {
        ...
        if (connection && connection.remoteFeatures.tn5250) {
          terminalsTab
            ...
            .addInput(`connectringStringFor5250`, `Connection string for 5250`, `The syntax for tn5250 is <code>[options] HOST[:PORT]</code>, default is <code>+uninhibited localhost</code><br />If your system is configured using telnet SSL, you need to specify <code>ssl</code> prefix before the connection string (examples <code>ssl:localhost</code> or <code>ssl:localhost:992</code>)<br/>You can also choose other options like <ul><li><code>+ruler</code>: Draw a ruler pointing to the cursor position</li><li><code>+uninhibited</code>: lets you get the cursor out of protected areas with any key</li></ul>`, { default: config.connectringStringFor5250 });
        } else if (connection) {
