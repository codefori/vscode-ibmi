    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(`code-for-ibmi.showAdditionalSettings`, async (server?: Server, tab?: string) => {
        ...
        featuresTab
          ...
          .addSelect(`autoUpdateDirectoryPermissions`, `Automatically update folder security`, ["ask", "always", "never"].map((value) => ({ text: value, description: value, value: value, selected: value === config.autoUpdateDirectoryPermissions})), `Automatically update home directory and .vscode directory permissions for better security. 'ask' will prompt each time, 'always' will update without prompting, 'never' will skip the check.`)
