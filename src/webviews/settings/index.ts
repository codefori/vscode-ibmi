...
const METADATA_SOURCE = [
  { value: "0", text: "0 (JDBC metadata)" },
  { value: "1", text: "1 (catalog metadata)" }
];
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(`code-for-ibmi.showAdditionalSettings`, async (server?: Server, tab?: string) => {
        ...
        mapepireTab
          ...
          .addCheckbox(`mapepireExtendedMetadata`, `Extended metadata`, `When enabled, requests extended JDBC metadata for SQL result sets.`, config.mapepireExtendedMetadata)
          .addSelect(`mapepireMetadataSource`, `Metadata source`,
            METADATA_SOURCE.map((source) => ({ value: source.value, text: source.text, description: source.text, selected: config.mapepireMetadataSource === source.value })),
            "Controls how metadata is retrieved by the JDBC driver.")
          .addCheckbox(`keepActionSpooledFiles`, `Keep actions spooled files`, `When enabled, spooled files generated by an action will not be deleted once the action is done.`, config.keepActionSpooledFiles)
          .addHorizontalRule()
        ...
        featuresTab
          ...
          .addSelect(`autoUpdateDirectoryPermissions`, `Automatically update folder security`, [
            { value: "ask", text: "will prompt each time", description: "ask", selected: "ask" === config.autoUpdateDirectoryPermissions },
            { value: "always", text: "will update without prompting", description: "always", selected: "always" === config.autoUpdateDirectoryPermissions },
            { value: "never", text: "will skip the check", description: "never", selected: "never" === config.autoUpdateDirectoryPermissions }
          ], `Automatically update home directory and .vscode directory permissions for better security.`)
        ...
        const tabs: ComplexTab[] = [
          { label: `Mapepire`, fields: mapepireTab.fields },
            ...
            if (page.data) {
              ...
              switch (button) {
                ...
                default:
                  ...
                  if (connection) {
                    if (config.mapepireJavaVersion !== 'default' && !connection.remoteFeatures[`jdk${config.mapepireJavaVersion.padEnd(2, "0")}`]) {
                      vscode.window.showWarningMessage(`The selected Java version is not installed in this IBM i. Install it (or right click on the connection and reload the server settings if it's actually installed). Mapepire will run with Java 8 until then.`);
                    }
                    if (restart) {
        ...
        if (server) {
          ...
          if (connection) {
            ...
            const ui = new CustomUI()
              ...
              .addCheckbox(`useSshAgent`, vscode.l10n.t(`Use SSH Agent`), vscode.l10n.t(`Enable this option to use the SSH agent for authentication. The agent must be running, and the identities must have been imported.`), stored.useSshAgent)
              .addHorizontalRule()
