  ...
  if (options.sep) {
    ...
    if (currentCommand) {
      ...
      const debugConfig = {
        ...
        "startBatchJobCommand": `SBMJOB CMD(${currentCommand}) INLLIBL(${options.libraries.libraryList.join(` `)}) CURLIB(${Tools.getCurLib(options.libraries.currentLibrary)}) JOBQ(QSYSNOMAX) MSGQ(*USRPRF) CPYENVVAR(*YES)`,
