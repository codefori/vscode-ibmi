...
import { parseErrors } from '../../errors/parser';

describe('Filter Tests', { concurrent: true }, () => {
  it('Basic test (CRTSQLRPGI, member)', () => {
    const lines = [
    ...
    expect(errors.has(filePath)).toBe(true);

    const fileErrors = errors.get(filePath);
    expect(fileErrors).toBeDefined();
    expect(fileErrors?.length).toBe(10);

    const errorA = fileErrors!.find(err => err.lineNum === 44);
    expect(errorA).toBeDefined();

    expect(errorA?.code).toBe(`SQL1001`);
    expect(errorA?.lineNum).toBe(44);
    expect(errorA?.toLineNum).toBe(44);
    expect(errorA?.column).toBe(0);
    expect(errorA?.toColumn).toBe(0);
    expect(errorA?.sev).toBe(30);
    expect(errorA?.text).toBe(`External file definition for EMPLOYEE not found.`);

    const lineErrors = fileErrors!.filter(err => err.lineNum === 104);
    expect(lineErrors.length).toBe(2);

    expect(lineErrors[0]?.code).toBe(`SQL0312`);
    expect(lineErrors[0]?.lineNum).toBe(104);
    expect(lineErrors[0]?.toLineNum).toBe(104);
    expect(lineErrors[0]?.column).toBe(16);
    expect(lineErrors[0]?.toColumn).toBe(16);
    expect(lineErrors[0]?.sev).toBe(30);
    expect(lineErrors[0]?.text).toBe(`Position 16 Variable EMPLOYEE not defined or not usable. Reason: No declaration for the variable exists, the declaration is not within the current scope, or the variable does not have an equivalent SQL data type.`);

    expect(lineErrors[1]?.code).toBe(`SQL0312`);
    expect(lineErrors[1]?.lineNum).toBe(104);
    expect(lineErrors[1]?.toLineNum).toBe(104);
    expect(lineErrors[1]?.column).toBe(25);
    expect(lineErrors[1]?.toColumn).toBe(25);
    expect(lineErrors[1]?.sev).toBe(30);
    expect(lineErrors[1]?.text).toBe(`Position 25 Variable FIRSTNME not defined or not usable. Reason: No declaration for the variable exists, the declaration is not within the current scope, or the variable does not have an equivalent SQL data type.`);
  });

  it('Basic test (CRTSQLRPGI, streamfile)', () => {
    const lines = [
    ...
    expect(errorA?.code).toBe(`SQL1001`);
    expect(errorA?.lineNum).toBe(41);
    expect(errorA?.toLineNum).toBe(41);
    expect(errorA?.column).toBe(0);
    expect(errorA?.toColumn).toBe(0);
    expect(errorA?.sev).toBe(30);
    expect(errorA?.text).toBe(`External file definition for EMPLOYEE not found.`);
    ...
    expect(lineErrors[0]?.code).toBe(`SQL0312`);
    expect(lineErrors[0]?.lineNum).toBe(106);
    expect(lineErrors[0]?.toLineNum).toBe(106);
    expect(lineErrors[0]?.column).toBe(25);
    expect(lineErrors[0]?.toColumn).toBe(25);
    expect(lineErrors[0]?.sev).toBe(30);
    expect(lineErrors[0]?.text).toBe(`Position 25 Variable EMPLOYEE not defined or not usable. Reason: No declaration for the variable exists, the declaration is not within the current scope, or the variable does not have an equivalent SQL data type.`);

    expect(lineErrors[1]?.code).toBe(`SQL0312`);
    expect(lineErrors[1]?.lineNum).toBe(106);
    expect(lineErrors[1]?.toLineNum).toBe(106);
    expect(lineErrors[1]?.column).toBe(34);
    expect(lineErrors[1]?.toColumn).toBe(34);
    expect(lineErrors[1]?.sev).toBe(30);
    expect(lineErrors[1]?.text).toBe(`Position 34 Variable FIRSTNME not defined or not usable. Reason: No declaration for the variable exists, the declaration is not within the current scope, or the variable does not have an equivalent SQL data type.`);
  });

  it('Long source file containing spaces (CRTSQLRPGI, streamfile)', () => {
    const lines = [
  ...
  it('nested copybook (CRTBNDRPG, streamfile)', () => {
    const lines = [
    ...
    expect(errors.has(filePath)).toBe(true);
    expect(errors.has(copybook_file_path)).toBe(true);
    expect(errors.has(nested_copybook_file_path)).toBe(true);

    // main file errors
    const fileErrors = errors.get(filePath);
    expect(fileErrors).toBeDefined();
    expect(fileErrors?.length).toBe(7);

    // copybook file errors
    const copybook_fileErrors = errors.get(copybook_file_path);
    expect(copybook_fileErrors).toBeDefined();
    expect(copybook_fileErrors?.length).toBe(2);

    // nested copybook file errors
    const nested_copybook_fileErrors = errors.get(nested_copybook_file_path);
    expect(nested_copybook_fileErrors).toBeDefined();
    expect(nested_copybook_fileErrors?.length).toBe(3);
  });

  it('filter errors (CRTSQLRPGI, streamfile)', () => {
    const lines = [
    ...
    const fileErrors = errors.get(filePath);
    expect(fileErrors).toBeDefined();
    expect(fileErrors?.length).toBe(25);

    // copybook file errors
    const copybook_fileErrors = errors.get(copybook_file_path);
    expect(copybook_fileErrors).toBeDefined();
    expect(copybook_fileErrors?.length).toBe(24);
  });
});
