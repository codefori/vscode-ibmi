...
import { db2Parse, makeid } from './tools';
...
export default class IBMi {
  ...
  tempRemoteFiles: {[path: string]: string};
  ...
  constructor() {
    ...
    this.config = new Configuration({name: `New config`});
    
  }

  /**
   * @param {ConnectionInfo} connectionObject
   * @returns {Promise<{success: boolean, error?: any}>} Was succesful at connecting or not.
   */
  async connect(connectionObject: ConnectionInfo) {
    try {
      connectionObject.keepaliveInterval = 35000;
      // Make sure we're not passing any blank strings, as node_ssh will try to validate it
      if (!connectionObject.privateKey) {connectionObject.privateKey = undefined;};
      ...
      return await vscode.window.withProgress({
        ...
        if (this.config.ifsShortcuts && this.config.ifsShortcuts.length === 0) {
          if (defaultHomeDir) {
        ...
        if (liblResult.code === 0) {
          ...
          if (libraryListString !== ``) {
            ...
            const currentLibraryConfig = this.config.currentLibrary;
            const libraryListConfig = this.config.libraryList;

            //If this is the first time the config is made, then these arrays will be empty
            if (!currentLibraryConfig || currentLibraryConfig && currentLibraryConfig.length === 0) {await this.config.set(`currentLibrary`, currentLibrary);}
            if (!libraryListConfig || libraryListConfig && libraryListConfig.length === 0) {await this.config.set(`libraryList`, this.defaultUserLibraries);}
          }
        }
        ...
        for (const feature of remoteApps) {
          try {
            ...
            const call = await this.paseCommand({
              command: `ls -p ${feature.path}${feature.specific || ``}`
            });
        ...
        if (this.remoteFeatures[`QZDFMDB2.PGM`]) {
          ...
          try {
            ...
            if (output.stdout) {
              const rows = db2Parse(output.stdout);
              rows.forEach(row => {
          ...
          try {
            ...
            if (output.stdout) {
              const [row] = db2Parse(output.stdout);
  ...
  remoteCommand(command: string, directory?: string) {
    ...
    return this.paseCommand({
      command: `system "` + command + `"`,
      directory
    });
  }

  async sendQsh(options: PaseCommand) {
    let qshCommand;
   ...
   */
  async paseCommand(options: PaseCommand, returnType = 0) {
   ...
   */
  async sendCommand(options: PaseCommand): Promise<CommandResponse> {
  ...
  getTempRemote(key: string) {
    ...
    if (this.tempRemoteFiles[key] !== undefined) {
      ...
      let value = path.posix.join(this.config.tempDir, `vscodetemp-${makeid()}`);
  ...
  parserMemberPath(inPath: string): MemberPathData {
    const result: MemberPathData = {
      library: '',
      file: '',
      member: '',
      basename: ''
    ...
    const path = inPath.startsWith(`/`) ? inPath.substring(1).toUpperCase().split(`/`) : inPath.toUpperCase().split(`/`);
    ...
    if (!result.library || !result.file || !result.basename) {
      throw new Error(`Invalid path: ${inPath}. Use format LIB/SPF/NAME.ext`);
    }
