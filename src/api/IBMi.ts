...
export default class IBMi {
  ...
  async connect(connectionObject: ConnectionData, options: { reconnecting?: boolean, reloadServerSettings?: boolean, progress?: ConnectionProgressFunc, warning?: ConnectionWarningHandle } = {}): Promise<ConnectionResult> {
    ...
    const quickConnect = (this.config.quickConnect === true && options.reloadServerSettings === false);
    ...
    updateProgress(`Checking library list configuration.`);
    ...
    updateProgress(`Checking temporary library configuration.`);
    ...
    updateProgress(`Checking temporary directory configuration.`);
    ...
    if (tempLibrarySet && this.config.autoClearTempData) {
      updateProgress(`Clearing temporary data.`);
    ...
    if (quickConnect === true && cachedServerSettings?.remoteFeaturesKeys && cachedServerSettings.remoteFeaturesKeys === Object.keys(this.remoteFeatures).sort().toString()) {
      ...
      updateProgress(`Checking installed components on host IBM i.`);
      ...
      for (const feature of remoteApps) {
        try {
          updateProgress(`Checking installed components on host IBM i: ${feature.path}`);
      ...
      updateProgress(`Checking installed components on host IBM i: Java`);
    ...
    if (this.sqlRunnerAvailable()) {
      ...
      if (quickConnect === true && cachedServerSettings?.aspInfo) {
        ...
        updateProgress(`Checking for ASP information.`);
        ...
        try {
          ...
          //Oh well
          updateProgress(`Failed to get ASP information.`);
        }
      }
      ...
      if (quickConnect === true && cachedServerSettings?.jobCcsid !== null && cachedServerSettings?.variantChars && cachedServerSettings?.userDefaultCCSID && cachedServerSettings?.qccsid) {
        ...
        updateProgress(`Fetching conversion values.`);
        ...
        try {
          ...
          updateProgress(`Fetching local encoding values.`);
      ...
      if (this.enableSQL) {
        updateProgress(`SQL program not installed. Disabling SQL.`);
      }
    }
    ...
    if (quickConnect === true && cachedServerSettings?.libraryListValidated === true) {
      ...
      if (this.config.libraryList) {
        updateProgress(`Validate configured library list`);
