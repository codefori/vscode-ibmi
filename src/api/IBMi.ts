      ...
      return await vscode.window.withProgress({
        ...
        if (!(await this.sendCommand({ command: "echo $PATH" })).stdout?.includes("/QOpenSys/pkgs/bin")) {
          ...
          if (await vscode.window.showWarningMessage(`/QOpenSys/pkgs/bin not found in $PATH`, {
            modal: true,
            detail: `Your $PATH shell environment variable does not include /QOpenSys/pkgs/bin, so Code for IBM i may not function correctly. Would you like to ${bashrcExists ? "update" : "create"} ${bashrcFile} to fix this now?`,
          }, `Yes`)) {
            delayedOperations.push(async () => {
              console.log(`${bashrcExists ? "update" : "create"} ${bashrcFile}`);
              if (!bashrcExists) {
