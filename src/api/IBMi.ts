    ...
    try {
      ...
      if (defaultHomeDir) {
        ...
        const getPermissions = async (path: string) => {
          const out = (await this.sendCommand({ command: `ls -ldL ${Tools.escapePath(path)} 2>/dev/null` })).stdout;
        ...
        const vscodeExistsResult = await this.sendCommand({
          command: `test -d ${Tools.escapePath(vscodeDir)} && echo "exists" || echo "notexists"`
        });
          ...
          if (!commandShellResult.stderr) {
            ...
            if (usesBash) {
              ...
              if ((!quickConnect() || !cachedServerSettings?.pathChecked)) {
                ...
                let bashrcExists = (await this.sendCommand({ command: `test -e ${Tools.escapePath(bashrcFile)}` })).code === 0;
                let reason;
