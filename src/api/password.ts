...
export namespace Password {
  ...
  export async function changePassword(connection: IBMi, oldPassword: string, newPassword: string) {
      ...
      const result = await connection.client?.execCommand(`system "CALL PGM(QSYS/QSYCHGPW) PARM(('*CURRENT  ') ('${oldPassword}') ('${newPassword}') (X'00000000') (${oldPassword.length} (*INT 4)) (0 (*INT 4)) (${newPassword.length} (*INT 4)) (0 (*INT 4)))"`);
      if(result?.code !== 0){
