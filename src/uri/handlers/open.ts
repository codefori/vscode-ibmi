...
async function loadParameters(query: querystring.ParsedUrlQuery, connection?: IBMi): Promise<Parameters> {
  ...
  if (connection && connectionData) {
    if (connectionData.name !== connection.currentConnectionName || (user && connection.currentUser.toLocaleUpperCase() !== user)) {
      const message = user ? l10n.t("You're currently connected to {0} with user profile {1}. Do you want to disconnect and switch to {2} with user profile {3}?", connection.currentHost, connection.currentUser, connectionData.name, user) :
