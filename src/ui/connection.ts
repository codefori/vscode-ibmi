  ...
  switch (error as ConnectionErrorCode) {
    ...
    case `home_directory_creation`:
      if (await window.showWarningMessage(`Home directory does not exist`, {
        ...
        const escapedHome = Tools.escapePath(data);
        const escapedHomeVscode = Tools.escapePath(`${data}/.vscode`);
        let mkHomeCmd = `mkdir -p ${escapedHome} && chown ${Tools.escapePath(connection.currentUser.toLowerCase())} ${escapedHome} && chmod 0750 ${escapedHome} && mkdir -p ${escapedHomeVscode} && chmod 0700 ${escapedHomeVscode}`;
    ...
    case `home_directory_permissions`:
      ...
      if (needsUpdate) {
        ...
        if (shouldUpdate) {
          ...
          const escapedHomeDir = Tools.escapePath(homeDir);
          const escapedVscodeDir = Tools.escapePath(`${homeDir}/.vscode`);
    ...
    case `invalid_bashrc`:
      ...
      if (await window.showWarningMessage(`${missingPath} not found in $PATH`, {
        ...
        if (!bashrcExists) {
          ...
          const escapedBashrcFile = Tools.escapePath(bashrcFile);
          const escapedUser = Tools.escapePath(connection.currentUser.toLowerCase());
