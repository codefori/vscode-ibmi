...
const node_ssh = __importStar(require("node-ssh"));
const os_1 = __importDefault(require("os"));
const path_1 = __importStar(require("path"));
const CompileTools_1 = require("./CompileTools");
const IBMiContent_1 = __importDefault(require("./IBMiContent"));
...
const configVars = __importStar(require("./configVars"));
const DebugConfiguration_1 = require("./configuration/DebugConfiguration");
const ConnectionManager_1 = require("./configuration/config/ConnectionManager");
const serverFile_1 = require("./configuration/serverFile");
const remoteApps = [
    {
        path: `/usr/bin/`,
...
class IBMi {
    ...
    static connectionManager = new ConnectionManager_1.ConnectionManager();
    static CCSID_NOCONVERSION = 65535;
    static CCSID_SYSVAL = -2;
    static bashShellPath = '/QOpenSys/pkgs/bin/bash';
    static locale = 'LC_ALL=EN_US.UTF-8';
    systemVersion = 0;
    qccsid = IBMi.CCSID_NOCONVERSION;
    userJobCcsid = IBMi.CCSID_SYSVAL;
    /** User default CCSID is job default CCSID */
    userDefaultCCSID = 0;
    sshdCcsid;
    componentManager = new manager_1.ComponentManager(this);
    configFiles = {
        settings: new serverFile_1.ConfigFile(this, `settings`, {})
    };
    /**
     ...
     */
    content = new IBMiContent_1.default(this);
    client;
    currentHost = ``;
    currentPort = 22;
    currentUser = ``;
    ...
    getConfigFile(id) {
        if (!this.configFiles[id]) {
            this.configFiles[id] = new serverFile_1.ConfigFile(this, id, {});
        }
        const configFile = this.configFiles[id];
        return configFile;
    }
    async loadRemoteConfigs() {
        for (const configFile in this.configFiles) {
            const currentConfig = this.configFiles[configFile];
            currentConfig.reset();
            try {
    ...
    get canUseCqsh() {
        return this.getComponent(cqsh_1.CustomQSh.ID) !== undefined;
    }
    /**
    ...
    async connect(connectionObject, options) {
        ...
        try {
            ...
            const delayedOperations = callbacks.onConnectedOperations ? [...callbacks.onConnectedOperations] : [];
            ...
            await this.client.connect({
                ...connectionObject,
                privateKeyPath: connectionObject.privateKeyPath ? Tools_1.Tools.resolvePath(connectionObject.privateKeyPath) : undefined,
                debug: connectionObject.sshDebug ? (message) => this.appendOutput(`\n[SSH debug] ${message}`) : undefined
            });
            ...
            const cachedServerSettings = IBMi.GlobalStorage.getServerSettingsCache(this.currentConnectionName);
            ...
            if (quickConnect() && cachedServerSettings?.remoteFeaturesKeys && cachedServerSettings.remoteFeaturesKeys === Object.keys(this.remoteFeatures).sort().toString()) {
                ...
                const javaCheck = async (root) => await this.content.testStreamFile(`${root}/bin/java`, 'x') ? root : undefined;
                [
            ...
            const remoteConnectionConfig = this.getConfigFile(`settings`);
            ...
            if (tempLibrarySet && this.config.autoClearTempData) {
                ...
                this.runCommand({
                    ...
                    if (result && result.stderr) {
                        const messages = Tools_1.Tools.parseMessages(result.stderr);
                ...
                this.sendCommand({
                    command: `rm -rf ${path_1.default.posix.join(this.config.tempDir, `vscodetemp*`)}`
                })
                    .then(result => {
                    // All good!
                })
            ...
            if ((!quickConnect || !cachedServerSettings?.debugConfigLoaded)) {
                if (this.debugPTFInstalled()) {
                    try {
                        const debugServiceConfig = await new DebugConfiguration_1.DebugConfiguration(this).load();
            ...
            if (this.sqlRunnerAvailable()) {
                ...
                const showCcsidWarning = (message) => {
    ...
    async ensureTempLibraryExists(fallbackTempLib) {
        ...
        if (createdTempLib.code === 0) {
            ...
            const messages = Tools_1.Tools.parseMessages(createdTempLib.stderr);
     ...
     */
    runCommand(data) {
        return CompileTools_1.CompileTools.runCommand(this, data);
    }
    ...
    async sendQsh(options) {
        ...
        if (this.canUseCqsh) {
            qshExecutable = this.getComponent(cqsh_1.CustomQSh.ID).installPath;
        }
    ...
    disconnect(failedToConnect = false) {
        if (this.client) {
    ...
    getTempRemote(key) {
        if (this.tempRemoteFiles[key] !== undefined) {
            ...
            let value = path_1.default.posix.join(this.config.tempDir, `vscodetemp-${Tools_1.Tools.makeid()}`);
            // console.log(`Using new temp: ${value}`);
            this.tempRemoteFiles[key] = value;
            return value;
        }
    }
    parserMemberPath(string, checkExtension) {
        ...
        const parsedPath = (0, path_1.parse)(upperCasedString);
    ...
    getLastDownloadLocation() {
        if (this.config?.lastDownloadLocation && (0, fs_1.existsSync)(Tools_1.Tools.fixWindowsPath(this.config.lastDownloadLocation))) {
    ...
    async setLastDownloadLocation(location) {
     ...
     */
    async withTempDirectory(process) {
        const tempDirectory = `${this.config?.tempDir || '/tmp'}/code4itemp${Tools_1.Tools.makeid(20)}`;
    ...
    getComponent(name, options) {
        return this.componentManager.get(name, options);
    }
    /**
     * Run SQL statements.
     ...
     */
    async runSQL(statements, options = {}) {
        ...
        if (QZDFMDB2) {
            ...
            let input = Tools_1.Tools.fixSQL(`${possibleChangeCommand}${statements}`, true);
            let returningAsCsv;
            let command = `${IBMi.locale} system "call QSYS/QZDFMDB2 PARM('-d' '-i' '-t')"`;
            let useCsv = options.forceSafe;
            // Use custom QSH if available
            if (this.canUseCqsh) {
                const customQsh = this.getComponent(cqsh_1.CustomQSh.ID);
            ...
            if (lastStmt) {
                        ...
                        if (fakeBindings[partsIndex] !== undefined) {
                            switch (typeof fakeBindings[partsIndex]) {
                                case `number`:
                                    ...
                                    break;
                                case `string`:
                                    lastStmt += Tools_1.Tools.bufferToUx(fakeBindings[partsIndex]);
                                    break;
                            }
                        }
                    }
                ...
                if (useCsv && (asUpper?.startsWith(`SELECT`) || asUpper?.startsWith(`WITH`))) {
                    const copyToImport = this.getComponent(copyToImport_1.CopyToImport.ID);
                    if (copyToImport) {
            ...
            if (output.stdout) {
                const fromStdout = Tools_1.Tools.db2Parse(output.stdout, input);
                if (returningAsCsv) {
                        ...
                        return (0, sync_1.parse)(csvContent, {
                            ...
                            onRecord(record) {
                                for (const key of Object.keys(record)) {
                                    record[key] = record[key] === ` ` ? `` : Tools_1.Tools.assumeType(record[key], key);
                                }
                                return record;
                            }
                        });
    ...
    async lookupLibraryIAsp(library) {
