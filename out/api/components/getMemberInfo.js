...
exports.GetMemberInfo = void 0;
const path_1 = require("path");
class GetMemberInfo {
    static ID = 'GetMemberInfo';
    procedureName = 'GETMBRINFO';
    currentVersion = 3;
    installedVersion = 0;
    reset() {
    ...
    async update(connection) {
        return connection.withTempDirectory(async (tempDir) => {
            const tempSourcePath = path_1.posix.join(tempDir, `getMemberInfo.sql`);
    ...
    static parseDateString(tsString) {
        if (!tsString) {
            return undefined;
        }
    ...
    async getMemberInfo(connection, library, sourceFile, member) {
    ...
    async getMultipleMemberInfo(connection, members) {
        const config = connection.getConfig();
        const tempLib = config.tempLibrary;
...
function getSource(library, name, version) {
    return Buffer.from([
...
//# sourceMappingURL=getMemberInfo.js.map