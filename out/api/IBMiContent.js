...
const Objects_1 = require("./import/Objects");
...
class IBMiContent {
    ...
    async getNotUTF8CCSID(attr, remotePath) {
    ...
    async convertToUTF8(iconv, from, to, ccsid) {
        const result = await this.ibmi.sendCommand({ command: `${iconv} -f IBM-${ccsid} -t UTF-8 ${Tools_1.Tools.escapePath(from)} > ${Tools_1.Tools.escapePath(to)}` });
        if (result.code === 0) {
     ...
     */
    async downloadStreamfileRaw(remotePath, localPath) {
     ...
     */
    async downloadStreamfile(remotePath, localPath) {
     ...
     */
    async writeStreamfileRaw(originalPath, content, encoding) {
        ...
        await writeFileAsync(tmpobj, content, { encoding: encoding });
    ...
    async downloadMemberContent(aspOrLibrary, libraryOrSourceFile, sourceFileOrMember, memberOrLocalPath, localPath) {
        ...
        const path = Tools_1.Tools.qualifyPath(library, sourceFile, member, asp, true);
            ...
            if (this.ibmi.dangerousVariants && new RegExp(`[${this.ibmi.variantChars.local}]`).test(path)) {
                ...
                try {
                    await this.ibmi.runSQL([
                        ...
                        `@QSYS/CPYTOSTMF FROMMBR('${Tools_1.Tools.qualifyPath("QTEMP", "QTEMPSRC", "TEMPMEMBER", undefined)}') TOSTMF('${tempRmt}') STMFOPT(*REPLACE) STMFCCSID(1208) DBFCCSID(${this.config.sourceFileCCSID});`
                    ].join("\n"));
                }
                catch (error) {
                    copyResult.code = -1;
    ...
    async uploadMemberContent(aspOrLibrary, libraryOrFile, sourceFileOrMember, memberOrContent, content) {
        ...
        try {
            ...
            if (this.ibmi.dangerousVariants && new RegExp(`[${this.ibmi.variantChars.local}]`).test(path)) {
                ...
                try {
                    await this.ibmi.runSQL([
                        ...
                        `@QSYS/CPYFRMSTMF FROMSTMF('${tempRmt}') TOMBR('${Tools_1.Tools.qualifyPath("QTEMP", "QTEMPSRC", "TEMPMEMBER", undefined)}') MBROPT(*REPLACE) STMFCCSID(1208) DBFCCSID(${this.config.sourceFileCCSID})`,
            ...
            if (copyResult.code === 0) {
                const messages = Tools_1.Tools.parseMessages(copyResult.stderr);
     ...
     */
    async getTable(library, file, member, deleteTable) {
        if (!member)
            member = file; //Incase mbr is the same file
        const tempRmt = this.getTempRemote(Tools_1.Tools.qualifyPath(library, file, member));
     ...
     */
    getQTempTable(prepareQueries, table) {
    ...
    async validateLibraryList(newLibl) {
        ...
        const sanitized = Tools_1.Tools.sanitizeObjNamesForPase(newLibl);
        const result = await this.ibmi.sendQsh({
            command: [
                `liblist -d ` + Tools_1.Tools.sanitizeObjNamesForPase(this.ibmi.defaultUserLibraries).join(` `),
     ...
     */
    async getObjectList(filters, sortOrder) {
        ...
        const singleEntry = filters.filterType !== 'regex' ? (0, Filter_1.singleGenericName)(filters.object) : undefined;
        const nameFilter = (0, Filter_1.parseFilter)(filters.object, filters.filterType);
        ...
        return objects.map(object => ({
            ...
            if (a.library.localeCompare(b.library) != 0) {
                ...
                return ((Objects_1.ObjectTypes.get(a.type) || 0) - (Objects_1.ObjectTypes.get(b.type) || 0)) || a.name.localeCompare(b.name);
            }
        });
    }
    /**
     ...
     */
    async getProgramExportImportInfo(library, name, type) {
     ...
     */
    async getModuleExports(library, name) {
        const outfile = Tools_1.Tools.makeid().toUpperCase();
        const results = await this.runStatements(`@DSPMOD MODULE(${library}/${name}) DETAIL(*EXPORT) OUTPUT(*OUTFILE) OUTFILE(QTEMP/${outfile})`, [
    ...
    async getMemberList(filter) {
        ...
        const memberFilter = (0, Filter_1.parseFilter)(filter.members, filter.filterType);
        ...
        const memberExtensionFilter = (0, Filter_1.parseFilter)(filter.extensions, filter.filterType);
        ...
        const statement = `with MEMBERS as (
        select
     ...
     * @returns
     */
    getMemberInfo(library, sourceFile, member) {
        const component = this.ibmi.getComponent(getMemberInfo_1.GetMemberInfo.ID);
        if (component) {
     ...
     */
    async getFileList(remotePath, sort = { order: "name" }, onListError) {
        ...
        if (STAT && SORT) {
            ...
            if (fileListResult.stdout !== '') {
                ...
                fileList.forEach(item => {
                    let auth, hardLinks, owner, group, size, modified, name;
                    ...
                    if (name !== `..` && name !== `.`) {
                        ...
                        items.push({
                            type: type,
                            name: name,
                            path: path_1.default.posix.join(remotePath, name),
                            size: Number(size),
            ...
            fileListResult = (await this.ibmi.sendCommand({
                command: `${this.ibmi.remoteFeatures.ls} -a -p -L ${sort.order === "date" ? "-t" : ""} ${(sort.order === 'date' && sort.ascending) ? "-r" : ""} ${Tools_1.Tools.escapePath(remotePath)}`
            }));
            if (fileListResult.stdout !== '') {
                ...
                fileList.split(`\n`)
                    ...
                    items.push({
                        ...
                        path: path_1.default.posix.join(remotePath, item)
                    });
                });
            }
        }
    ...
    async memberResolve(member, files) {
        const inAmerican = (s) => { return this.ibmi.sysNameInAmerican(s); };
        const inLocal = (s) => { return this.ibmi.sysNameInLocal(s); };
        ...
        const pathList = files
            ...
            if (asp && asp.length > 0) {
                return [
                    Tools_1.Tools.qualifyPath(inAmerican(file.library), inAmerican(file.name), inAmerican(member), asp, true),
                    Tools_1.Tools.qualifyPath(inAmerican(file.library), inAmerican(file.name), inAmerican(member), undefined, true)
                ].join(` `);
            }
            else {
                return Tools_1.Tools.qualifyPath(inAmerican(file.library), inAmerican(file.name), inAmerican(member), undefined, true);
            }
        })
            .join(` `)
            .toUpperCase();
        ...
        if (result.code === 0) {
            ...
            if (firstMost) {
                try {
                    const simplePath = inLocal(Tools_1.Tools.unqualifyPath(firstMost));
    ...
    async objectResolve(object, libraries) {
        ...
        if (result.code === 0) {
            const firstMost = result.stdout;
            if (firstMost) {
                const lib = this.ibmi.sysNameInLocal(Tools_1.Tools.unqualifyPath(firstMost));
                return lib.split('/')[1];
            }
        }
        return undefined;
    }
    async streamfileResolve(names, directories) {
        const command = `for f in ${directories.flatMap(dir => names.map(name => `"${Tools_1.Tools.escapePath(path_1.default.posix.join(dir, name), true)}"`)).join(` `)}; do if [ -f "$f" ]; then echo $f; break; fi; done`;
        const result = await this.ibmi.sendCommand({
            command,
        });
    ...
    async isDirectory(remotePath) {
        return (await this.ibmi.sendCommand({
            command: `cd ${Tools_1.Tools.escapePath(remotePath)}`
        })).code === 0;
    }
    async checkObject(object, authorities = [`*NONE`]) {
    ...
    async testStreamFile(path, right) {
        return (await this.ibmi.sendCommand({ command: `test -${right} ${Tools_1.Tools.escapePath(path)}` })).code === 0;
    }
    ...
    async getAttributes(path, ...operands) {
        ...
        if (assumeMember) {
            ...
            target = Tools_1.Tools.qualifyPath(localPath.library, localPath.name, localPath.member || '', localPath.asp || '', true);
        }
        else {
            target = localPath;
        }
    ...
    async countMembers(path) {
        return this.countFiles(this.ibmi.sysNameInAmerican(Tools_1.Tools.qualifyPath(path.library, path.name, undefined, path.asp)));
    }
    async countFiles(directory) {
        return Number((await this.ibmi.sendCommand({ command: `cd "${directory}" && (ls | wc -l)` })).stdout.trim());
    }
    async checkUserSpecialAuthorities(authorities, user) {
        const profile = (user || this.ibmi.currentUser).toLocaleUpperCase();
        const [row] = await this.ibmi.runSQL(`select trim(coalesce(usr.special_authorities,'') concat ' ' concat coalesce(grp.special_authorities, '')) AUTHORITIES ` +
        ...
        const userAuthorities = row?.AUTHORITIES ? String(row.AUTHORITIES).split(" ").filter(Boolean).filter(Tools_1.Tools.distinct) : [];
    ...
    `;
        ...
        return Number(result.CCSID === IBMi_1.default.CCSID_NOCONVERSION ? result.DEFAULT_CCSID : result.CCSID);
    }
    async getSysEnvVars() {
     ...
     */
    async createStreamFile(path) {
        path = Tools_1.Tools.escapePath(path);
    ...
    uploadFiles(files, options) {
        return this.ibmi.client.putFiles(files.map(f => { return { local: Tools_1.Tools.fileToPath(f.local), remote: f.remote }; }), options);
    }
    downloadFile(localFile, remoteFile) {
        return this.ibmi.client.getFile(Tools_1.Tools.fileToPath(localFile), remoteFile);
    }
    uploadDirectory(localDirectory, remoteDirectory, options) {
        return this.ibmi.client.putDirectory(Tools_1.Tools.fileToPath(localDirectory), remoteDirectory, options);
    }
    downloadDirectory(localDirectory, remoteDirectory, options) {
        return this.ibmi.client.getDirectory(Tools_1.Tools.fileToPath(localDirectory), remoteDirectory, options);
    }
    /**
     ...
     */
    async copy(paths, toPath) {
        paths = Array.isArray(paths) ? paths : [paths];
        const toPathIsDir = await this.isDirectory(Tools_1.Tools.escapePath(toPath));
     ...
     */
    async move(paths, toDirectory) {
        paths = Array.isArray(paths) ? paths : [paths];
        return await this.ibmi.runCommand({ command: `mv ${paths.map(path => Tools_1.Tools.escapePath(path)).join(" ")} ${Tools_1.Tools.escapePath(toDirectory)}`, environment: "qsh" });
    }
}
exports.default = IBMiContent;
//# sourceMappingURL=IBMiContent.js.map