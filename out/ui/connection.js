...
exports.handleConnectionResults = exports.messageCallback = void 0;
const vscode_1 = require("vscode");
function messageCallback(type, message) {
    switch (type) {
        case `info`:
...
async function handleConnectionResults(connection, error, data) {
    switch (error) {
        case `shell_config`:
            const chosen = await vscode_1.window.showInformationMessage(`Error in shell configuration!`, {
                detail: [
            ...
            if (chosen === `Read more`) {
                vscode_1.commands.executeCommand(`open`, `https://codefori.github.io/docs/tips/setup/#error-in-shell-configuration`);
            }
            break;
        case `home_directory_creation`:
            if (await vscode_1.window.showWarningMessage(`Home directory does not exist`, {
                modal: true,
                ...
                if (0 === mkHomeResult.code) {
                    ...
                    await vscode_1.window.showWarningMessage(`Error creating home directory (${data}):\n${mkHomeErrs}.\n\n Code for IBM i may not function correctly. Please contact your system administrator.`, { modal: true });
                    return false;
                }
            }
            break;
        case `QCPTOIMPF_exists`:
            vscode_1.window.showWarningMessage(`The data area QSYS/QCPTOIMPF exists on this system and may impact Code for IBM i functionality.`, {
                ...
                switch (choice) {
                    case `Delete`:
                        connection.runCommand({
                            ...
                            .then((result) => {
                            if (result?.code === 0) {
                                vscode_1.window.showInformationMessage(`The data area QSYS/QCPTOIMPF has been deleted.`);
                            }
                            else {
                                vscode_1.window.showInformationMessage(`Failed to delete the data area QSYS/QCPTOIMPF. Code for IBM i may not work as intended.`);
                            }
                        });
                        break;
                    case `Read more`:
                        vscode_1.env.openExternal(vscode_1.Uri.parse(`https://github.com/codefori/vscode-ibmi/issues/476#issuecomment-1018908018`));
                        break;
                }
            });
            break;
        case `QCPFRMIMPF_exists`:
            vscode_1.window.showWarningMessage(`The data area QSYS/QCPFRMIMPF exists on this system and may impact Code for IBM i functionality.`, {
                modal: false,
            }, `Delete`, `Read more`).then(choice => {
                switch (choice) {
                    case `Delete`:
                        connection.runCommand({
                            ...
                            .then((result) => {
                            if (result?.code === 0) {
                                vscode_1.window.showInformationMessage(`The data area QSYS/QCPFRMIMPF has been deleted.`);
                            }
                            else {
                                vscode_1.window.showInformationMessage(`Failed to delete the data area QSYS/QCPFRMIMPF. Code for IBM i may not work as intended.`);
                            }
                        });
                        break;
                    case `Read more`:
        ...
        case `default_not_bash`:
            vscode_1.window.showInformationMessage(`IBM recommends using bash as your default shell.`, `Set shell to bash`, `Read More`).then(async (choice) => {
                switch (choice) {
                    case `Set shell to bash`:
                        ...
                        if (!commandSetBashResult.stderr) {
                            vscode_1.window.showInformationMessage(`Shell is now bash! Reconnect for change to take effect.`);
                        }
                        else {
                            vscode_1.window.showInformationMessage(`Default shell WAS NOT changed to bash.`);
                        }
                        break;
                    case `Read More`:
                        vscode_1.env.openExternal(vscode_1.Uri.parse(`https://ibmi-oss-docs.readthedocs.io/en/latest/user_setup/README.html#step-4-change-your-default-shell-to-bash`));
                        break;
                }
            });
            break;
        case `invalid_bashrc`:
            ...
            if (await vscode_1.window.showWarningMessage(`${missingPath} not found in $PATH`, {
                modal: true,
                ...
                if (!bashrcExists) {
                    ...
                    if (createBashrc.code !== 0) {
                        vscode_1.window.showWarningMessage(`Error creating ${bashrcFile}):\n${createBashrc.stderr}.\n\n Code for IBM i may not function correctly. Please contact your system administrator.`, { modal: true });
                    }
                }
                else {
                    try {
                        ...
                        if (!replaced) {
                            bashrcContent.push("", "# Generated by Code for IBM i", "export PATH=/QOpenSys/pkgs/bin:$PATH:/QOpenSys/usr/bin:/usr/bin");
                        }
                        await content.writeStreamfileRaw(bashrcFile, bashrcContent.join("\n"), `utf8`);
                    }
                    catch (error) {
                        vscode_1.window.showWarningMessage(`Error modifying PATH in ${bashrcFile}):\n${error}.\n\n Code for IBM i may not function correctly. Please contact your system administrator.`, { modal: true });
                    }
                }
            }
            break;
        case `invalid_temp_lib`:
            vscode_1.window.showWarningMessage(`Code for IBM i will not function correctly until the temporary library has been corrected in the settings.`, `Open Settings`)
                .then(result => {
                switch (result) {
                    case `Open Settings`:
                        vscode_1.commands.executeCommand(`code-for-ibmi.showAdditionalSettings`);
                        break;
                }
            });
            break;
        case `ccsid_warning`:
            vscode_1.window.showWarningMessage(data, `Show documentation`).then(choice => {
                if (choice === `Show documentation`) {
                    vscode_1.commands.executeCommand(`open`, `https://codefori.github.io/docs/tips/ccsid/`);
                }
            });
            break;
    }
...
exports.handleConnectionResults = handleConnectionResults;
